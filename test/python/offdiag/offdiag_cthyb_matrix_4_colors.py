...
S = Solver(beta = beta, gf_struct = [ ('up',2), ('down',2) ])
...
for spin, g0 in S.G0 :
    ...
    g0[1,0] =  - alpha[spin]*V**2 *(inverse(iOmega_n -e_h)  + inverse(iOmega_n + e_h))   
    g0[1,1] = iOmega_n + mu - split/2 - V**2 *(inverse(iOmega_n -e_h)  + inverse(iOmega_n + e_h)) 
    g0[0,1] = - alpha[spin]* V**2 *(inverse(iOmega_n -e_h)  + inverse(iOmega_n + e_h))
    g0.invert()
# Run the solver. The result will be in S.G
S.solve(H_local = U * N('up',1) * N('up',2)+  U * N('down',1) * N('down',2),   # Local Hamiltonian 
        quantum_numbers = {                      # Quantum Numbers 
          'Nup1' : N('up',1),                     # Operators commuting with H_Local
          'Nup2' : N('up',2) ,
          'Ndown1' : N('down',1),                     # Operators commuting with H_Local
          'Ndown2' : N('down',2) ,
          },          
        ...
        measured_operators = {                   # Operators to be averaged
          'Nimp_up' : N('up',1)+N('up',2) ,
          'Nimp_down' : N('down',1)+N('down',2) }
        )
...
if mpi.is_master_node():
  Results = HDFArchive("matrix_4_colors.h5",'w')
  Results["G"] = S.G
  Results["Gl"] = S.G_legendre
  Results["Nimp_up"] = S.measured_operators_results['Nimp_up']
  Results["Nimp_down"] = S.measured_operators_results['Nimp_down']
  Results["G0"] = S.G0
