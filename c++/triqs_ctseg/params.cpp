...
namespace triqs_ctseg {
  ...
  void h5_write(h5::group h5group, std::string subgroup_name, solve_params_t const &c) {
    ...
    h5_write(grp, "measure_chi3", c.measure_chi3);
    h5_write(grp, "measure_chi4", c.measure_chi4);
    h5_write(grp, "dlr_wmax", c.dlr_wmax);
    h5_write(grp, "dlr_eps", c.dlr_eps);
    h5_write(grp, "dlr2d_compress_grid", c.dlr2d_compress_grid);
    h5_write(grp, "n_w_chi4_f", c.n_w_chi4_f);
    h5_write(grp, "n_w_chi4_b", c.n_w_chi4_b);
    h5_write(grp, "nfft_buf_size", c.nfft_buf_size);
    h5_write(grp, "nfft_tol", c.nfft_tol);
  ...
  void h5_read(h5::group h5group, std::string subgroup_name, solve_params_t &c) {
    ...
    h5::try_read(grp, "measure_chi3", c.measure_chi3);
    h5::try_read(grp, "measure_chi4", c.measure_chi4);
    h5::try_read(grp, "dlr_wmax", c.dlr_wmax);
    h5::try_read(grp, "dlr_eps", c.dlr_eps);
    h5::try_read(grp, "dlr2d_compress_grid", c.dlr2d_compress_grid);
    h5::try_read(grp, "n_w_chi4_f", c.n_w_chi4_f);
    h5::try_read(grp, "n_w_chi4_b", c.n_w_chi4_b);
    h5::try_read(grp, "nfft_buf_size", c.nfft_buf_size);
    h5::try_read(grp, "nfft_tol", c.nfft_tol);
