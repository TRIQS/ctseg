      ...
      if (not sl.empty()) {
        for (int i = 0; i < sl.size(); ++i) {
          if (i != sl.size() - 1) {
            ...
            ALWAYS_EXPECTS(not is_cyclic(sl[i]),
                           "Segment in color {} at position {} should not by cyclic in config \n{}", c, i,
                           config); // only last segment can be cyclic
          }
          ALWAYS_EXPECTS((sl[i].tau_cdag != sl[i].tau_c),
                         "Degenerate segment in color {} at position {} in config \n{}", c, i, config);
        }
      }
    }
    LOG("Segments OK.");
          ...
          for (auto const &seg : sl) {
            ...
            if (not seg.J_cdag and !is_full_line(seg)) {
              ALWAYS_EXPECTS(D.size() != 0,
                             "Det error, block {}: there is a hybridized cdag but det is empty. Config: {}", bl,
                             config);
